...
class Herder(OasisAgent):
	...
	def getNewHerderPosition(self):
		for j in range(self.getWorld()._config._size._width-1, -1, -1):
			for i in range(self.getWorld()._config._size._height-1, -1, -1):
...
class Farmer(OasisAgent):
	...
	def getNewFarmerPosition(self):
		for j in range(0,self.getWorld()._config._size._width):
			for i in range(0,self.getWorld()._config._size._height):
				possiblePosition = Point2DInt(i,j)
...
class Oasis(World):
	...
	def createAgents(self):
		print('creating farmers:',self._config._initFarmers,'and herders:',self._config._initHerders)
		for i in range (0, self._config._initFarmers):
			...
			newAgent.position = Point2DInt(int(i%self._config._size._width),int(i/self._config._size._height))
			self._numFarmers += 1
		...
		for i in range (0, self._config._initHerders):
			...
			newAgent.position = Point2DInt(int(self._config._size._width-1-i%self._config._size._height),int(self._config._size._width-1-i/self._config._size._height))
			self._numHerders += 1
	...
	def resolveConflicts( self, position ):
		...
		indexOfOpportunity = self._numFields / (self._config._size._width*self._config._size._height)
		...
		print('\tratio of strength:',ratioOfStrengths,'incentive migration:',incentiveForMigration,' index of opportunity: ',indexOfOpportunity,'num fields:',self._numFields,'size:',self._config._size)
	...
	def checkConflicts(self):
		...
		for index._y in range(0,self._config._size._width):
			for index._x in range(0,self._config._size._height):
	...
	def shufflePositions(self):
		...
		for i in range(0, len(listFarmers)):
			...
			# next column
			if(index._x<(self._config._size._width-1)):
				index._x +=1
			# next row
			else:
			 	index._x = 0
		...
		listHerders = []
		index = Point2DInt(0,0)	
		for index._y in range(self._config._size._height-1, -1, -1):
			for index._x in range(self._config._size._width-1, -1, -1):
		...
		index = Point2DInt(self._config._size._width-1,self._config._size._height-1)
		for i in range(0, len(listHerders)):
			...
			# next row
			 	...
			 	index._x = self._config._size._width-1
			 	index._y -= 1
...
def main():
	...
	print('size: ',config._size,'num steps:',config._numSteps)
