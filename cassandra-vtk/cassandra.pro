...
#INCLUDEPATH += /usr/local/include/vlGraphics/
#INCLUDEPATH += /usr/local/include/vlCore/
#INCLUDEPATH += ../../VTK/GUISupport/Qt/
# Required when installing qwt-6.0.0 from sources
INCLUDEPATH += /usr/local/qwt-6.0.0/include/
CONFIG += qt

# Required when installing qwt-6.0.0 from sources
LIBS += -fopenmp -L../pandora/ -lpandora -L/usr/local/qwt-6.0.0/lib/ -L/usr/local/hdf5/lib/ -lqwt -lhdf5 -lmpich -lmpi -lGL -lGLU -lQtOpenGL -lIL -ltinyxml -lQVTK
...
LIBS += \
    /usr/lib/libvtkIO.so.5.8
LIBS += \
    /usr/local/lib/libVLGraphics.so
LIBS += \
    /usr/local/lib/libVLCore.so
LIBS += \
    /usr/local/lib/libVLQt4.so.2011.9
