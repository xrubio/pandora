...
SpacePartition::SpacePartition( const Simulation & simulation, const int & overlap, World & world ) : _worldPos(-1,-1), _finalize(true), _overlap(overlap), _world(world), _id(-1), _numTasks(1), _localRasterSize(0,0) 
{
}
...
void SpacePartition::init( int argc, char *argv[] )
{
	...
	_localRasterSize._x = _world.getSize()._x/sqrt(_numTasks);
	_localRasterSize._y = _world.getSize()._y/sqrt(_numTasks);
...
{
	int subfieldSizeX = _localRasterSize._x/2;
	int subfieldSizeY = _localRasterSize._y/2;
	if(_overlap*2>subfieldSizeX || _overlap*2>subfieldSizeY)
	{
		std::stringstream oss;
		oss << "SpacePartition::checkOverlapSize- subfield sizes: " << subfieldSizeX << "/" << subfieldSizeY << " from global: " << _world.getSize() << " and local: " << _localRasterSize << " must be at least twice the value of overlap: " << _overlap << " to avoid conflicts between non adjacent subfields";
		throw Exception(oss.str());
	}
}

void SpacePartition::stablishPosition()
{
...
void SpacePartition::stepSection( const int & sectionIndex )
{
	...
	log_DEBUG(logName.str(), _world.getWallTime() << " beginning step: " << _step << " section: " << sectionIndex);
	...
	for(size_t i=0; i<agentsToExecute.size(); i++)
	{
		...
		log_DEBUG(logName.str(), _world.getWallTime() << " agent: " << agent << " being executed at index: " << sectionIndex << " of task: "<< _id << " in step: " << _step );
		...
		if(!_ownedArea.isInside(agent->getPosition()) && !willBeRemoved(agent))
		{
			...
			log_DEBUG(logName.str(), _world.getWallTime() <<  "putting agent: " << agent << " to overlap");
		}
		else
		{
			log_DEBUG(logName.str(), _world.getWallTime() << " finished agent: " << agent);
		}
		_executedAgentsHash.insert(make_pair(agent->getId(), agent));
		numExecutedAgents++;
		log_DEBUG(logName.str(), _world.getWallTime()  << " num executed agents: " << numExecutedAgents );
	}
	log_DEBUG(logName.str(), _world.getWallTime()  << " sending agents in section: " << sectionIndex << " and step: " << _step);
	sendAgents(agentsToSend);
	log_DEBUG(logName.str(), _world.getWallTime() << " has finished section: " << sectionIndex << " and step: " << _step);
	
	log_DEBUG(logName.str(), _world.getWallTime() << " executed step: " << _step << " section: " << sectionIndex << " in zone: " << _sections[sectionIndex] << " with num executed agents: " << numExecutedAgents << " total agents: " << _agents.size() << " and overlap agents: " << _overlapAgents.size());
}
...
void SpacePartition::sendAgents( AgentsList & agentsToSend )
{
	...
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " sendAgent: " << agentsToSend.size() << " agents");
	...
	{
		log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " sendAgent - checking mpi type: " << itType->first );
		...
		{	
			...
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " sendAgent - sending num agents: " << numAgents << " to: " << _neighbors[i]);
			...
			while(it!=agentsToNeighbors[i].end())
			{
				...
				log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " sendAgent - sending agent: " << *it << " to: " << _neighbors[i] );
	...
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " sendAgent -  end checking agents to send: " << agentsToSend.size());
}
...
void SpacePartition::sendOverlapZones( const int & sectionIndex, const bool & entireOverlap )
{
	...
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " sendOverlapZones");
	...
	for(size_t d=0; d<_world.getNumberOfRasters(); d++)
	{
		...
		{
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " index: " << d << " not sending");
			continue;
		}
		log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " sending raster: " << d);
		for(size_t i=0; i<neighborsToUpdate.size(); i++)
		{
			...
			if(entireOverlap)
			{
				...
				log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " send entire overlap: " << send->_overlap << " to: " << neighborsToUpdate[i]);
			}
			else
			{
				send->_overlap = getInternalOverlap(neighborsToUpdate[i]);
				log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " send partial overlap: " << send->_overlap << " to: " << neighborsToUpdate[i]);
			}
			const Rectangle<int> & overlapZone = send->_overlap;
			send->_data.resize(overlapZone._size._x * overlapZone._size._y);
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " will send overlap to: " << neighborsToUpdate[i] << " with size: " << send->_data.size() << " and zone: " << overlapZone);
			for(size_t n=0; n<send->_data.size(); n++)
			{
				...
				log_EDEBUG(logName.str(), "\t" << _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " send index: " << index << " in global pos: " << index+_overlapBoundaries._origin << " value: " << send->_data.at(n));
			}
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " raster: " << d << " will be sent");
			...
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " raster: " << d << " data sent to: " << _neighbors[i]);
		}
		log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " raster: " << d << " sent");
	}
	log_DEBUG(logName.str(), _world.getWallTime() << " step: "  << "/" << sectionIndex << _step << " sendOverlapZones ended");
}
...
void SpacePartition::sendMaxOverlapZones()
{
	std::stringstream logName;
	logName << "MPI_raster_world_" << _id;
	log_DEBUG(logName.str(), _world.getWallTime() << " step: "  << _step << " sendMaxOverlapZones");
	for(size_t d=0; d<_world.getNumberOfRasters(); d++)
	{
		if(!_world.getStaticRasterIndex(d) || !_world.getDynamicRasterIndex(d))
		{
		...
		log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " sending max raster: " << d);
		for(size_t i=0; i<_neighbors.size(); i++)
		{
			MpiOverlap * send = new MpiOverlap;
			send->_overlap = getInternalOverlap(_neighbors[i]);
			send->_data.resize(send->_overlap._size._x * send->_overlap._size._y);
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " will send max overlap of: " << d << " to: " << _neighbors[i] << " with size: " << send->_data.size() << " and zone: " << send->_overlap << " to " << _neighbors[i]);
			const Rectangle<int> & overlapZone = send->_overlap;
			for(size_t n=0; n<send->_data.size(); n++)
			{
				Point2D<int> index(overlapZone._origin._x+n%overlapZone._size._x, overlapZone._origin._y+n/overlapZone._size._x);
				send->_data.at(n) = _world.getDynamicRaster(d).getMaxValueAt(index);
				log_EDEBUG(logName.str(), "\t" << _world.getWallTime() << " step: " << _step << " send index: " << index << " in global pos: " << index+_overlapBoundaries._origin << " max value: " << send->_data.at(n));
			}
			MPI_Isend(&send->_data[0], send->_data.size(), MPI_INTEGER, _neighbors[i], eRasterMaxData, MPI_COMM_WORLD, &send->_request);
			_sendRequests.push_back(send);
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " raster: " << d << " max data sent to: " << _neighbors[i]);
		}
		log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " raster: " << d << " max data sent");
	}
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " sendMaxOverlapZones ended");
}
...
void SpacePartition::sendGhostAgents( const int & sectionIndex )
{
	std::stringstream logName;
	logName << "MPI_agents_world_" << _id;
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " send ghost agents for section index: " << sectionIndex );
	...
	for(size_t i=0; i<_neighbors.size(); i++)
	{
		...
		{
			neighborsToUpdate.push_back(_neighbors[i]);
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " section index: " << sectionIndex << " will send overlap to: " << _neighbors[i]);
		}
	}
	...
	for(MpiFactory::TypesMap::iterator itType=MpiFactory::instance()->beginTypes(); itType!=MpiFactory::instance()->endTypes(); itType++)
	{
		log_DEBUG(logName.str(),  _world.getWallTime() << " step: " << _step << " section index: " << sectionIndex << " checking type: " << itType->first );
		MPI_Datatype * agentType = itType->second;
		...
		for(size_t i=0; i<neighborsToUpdate.size(); i++)
		{
			...
			log_DEBUG(logName.str(),  _world.getWallTime() << " step: " << _step << " sending num ghost agents: " << numAgents << " to : " << neighborsToUpdate[i] << " in step: " << _step << " and section index: " << sectionIndex );
			...
			for(AgentsList::iterator it=agentsToNeighbors[i].begin(); it!=agentsToNeighbors[i].end(); it++)
			{
				Agent * agent = *it;
				void * package = agent->fillPackage();
				log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " sending ghost agent: " << *it << " from: " << _id << " to: " << neighborsToUpdate[i]);
	...
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " send ghost agents for section index: " << sectionIndex << " finished");
}
...
void SpacePartition::receiveGhostAgents( const int & sectionIndex )
{
	std::stringstream logName;
	logName << "MPI_agents_world_" << _id;
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " receive ghost agents for section index: " << sectionIndex );
	...
	for(MpiFactory::TypesMap::iterator itType=MpiFactory::instance()->beginTypes(); itType!=MpiFactory::instance()->endTypes(); itType++)
	{
		...
		for(size_t i=0; i<neighborsToUpdate.size(); i++)
		{
			...
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " has received message from " << neighborsToUpdate[i] << ", num ghost agents: " << numAgentsToReceive );
			for(size_t j=0; j<numAgentsToReceive; j++)
			{
				...
				log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " has received ghost agent: " << agent << " number: " << j << " from: " << neighborsToUpdate[i] << " in section index: " << sectionIndex << " and step: " << _step );
				delete package;
				...
				{
					log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " has received update of own agent: " << *it << " in step: " << _step );
			...
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " num ghost agents sent for neighbor: " << neighborsToUpdate[i] << " in section index: " << sectionIndex << ": " << newGhostAgents.size());
			...
			while(it!=_overlapAgents.end())
			{
				...
				if(agent->isType(itType->first))
				{
					...
					{
						log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " in section index: " << sectionIndex << " with overlap zone: " << overlapZone << " erasing agent: " << *it);
						it = _overlapAgents.erase(it);
					}
					else
					{
						log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " in section index: " << sectionIndex <<  " with overlap zone: " << overlapZone << " maintaining agent: " << *it );
						it++;
					}
				}
				else
			...
			{
				log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " in section index: " << sectionIndex << " adding ghost agent: " << *it );
				_overlapAgents.push_back(*it);
			}
		}
	}
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " receive ghost agents for section index: " << sectionIndex << " finished");
...
void SpacePartition::receiveAgents( const int & sectionIndex )
{
	std::stringstream logName;
	logName << "MPI_agents_world_" << _id;
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " receiving agents for section index: " << sectionIndex);
	for(MpiFactory::TypesMap::iterator itType=MpiFactory::instance()->beginTypes(); itType!=MpiFactory::instance()->endTypes(); itType++)
	{
		log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " receiveAgent - checking mpi type: " << itType->first);
		MPI_Datatype * agentType = itType->second;

		for(size_t i=0; i<_neighbors.size(); i++)
		{
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " receiveAgent - checking mpi type: " << itType->first << " for neighbor: " << _neighbors[i] );
			...
			log_DEBUG(logName.str(), _world.getWallTime() <<  " receiveAgents - received message from " << _neighbors[i] << ", num agents: " << numAgentsToReceive);
			for(size_t j=0; j<numAgentsToReceive; j++)
			{
				void * package = MpiFactory::instance()->createDefaultPackage(itType->first);
				...
				Agent * agent = MpiFactory::instance()->createAndFillAgent(itType->first, package);
				log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " receiveAgents - received agent: " << agent << " number: " << j << " from: " << _neighbors[i]);
				delete package;
...
void SpacePartition::receiveOverlapData( const int & sectionIndex, const bool & entireOverlap )
{
	std::stringstream logName;
	logName << "MPI_raster_world_" << _id;
	log_DEBUG(logName.str(), _world.getWallTime() << " step: "  << "/" << sectionIndex << _step << " receiveOverlapData");
	...
	{	
		...
		log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " receiving raster: " << d);
		for(size_t i=0; i<neighborsToUpdate.size(); i++)
		{
			...
			if(entireOverlap)
			{
				...
				log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " receive entire overlap: " << receive->_overlap << " from " << neighborsToUpdate[i]);
			}
			else
			{
				receive->_overlap = getExternalOverlap(neighborsToUpdate[i]);
				log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " receive external overlap: " << receive->_overlap << " from " << neighborsToUpdate[i]);
			}
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " AAA will receive overlap from: " << neighborsToUpdate[i] << " with size: " << receive->_data.size() << " and zone: " << receive->_overlap );
			receive->_data.resize(receive->_overlap._size._x*receive->_overlap._size._y);
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " will receive overlap from: " << neighborsToUpdate[i] << " with size: " << receive->_data.size() << " and zone: " << receive->_overlap );
		...
		log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " raster: " << d << " received");
	}
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << "/" << sectionIndex << " receiveOverlapData ended");
}
...
void SpacePartition::receiveMaxOverlapData()
{
	std::stringstream logName;
	logName << "MPI_raster_world_" << _id;
	log_DEBUG(logName.str(), _world.getWallTime() << " step: "  << _step << " receiveMaxOverlapData");
	// for each raster, we receive data from all the active neighbors
	for(size_t d=0; d<_world.getNumberOfRasters(); d++)
	{
		if(!_world.getStaticRasterIndex(d) || !_world.getDynamicRasterIndex(d))
		...
		log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " receiving max raster: " << d);
		for(size_t i=0; i<_neighbors.size(); i++)			
		{
			MpiOverlap* receive = new MpiOverlap;
			receive->_rasterName = _world.getRasterName(d);
			...
			log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " will receive max overlap to: " << _neighbors[i] << " with size: " << receive->_data.size() << " and zone: " << receive->_overlap << " from " << _neighbors[i]);
			MPI_Irecv(&receive->_data[0], receive->_data.size(), MPI_INTEGER, _neighbors[i], eRasterMaxData, MPI_COMM_WORLD, &receive->_request);
			_receiveRequests.push_back(receive);
		}
		log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step  << " raster: " << d << " max data received");
	}
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " receiveMaxOverlapData ended");
}
	...
	while(_sendRequests.size()!=0)
	{
		...
		while(it!=_sendRequests.end())
		{
			...
			if(finished)
			{
				...
				log_DEBUG(logName.str(), _world.getWallTime() << " request finished, lacking: " << _sendRequests.size() << " requests");
				delete send;
			}
		}
	}
	...
	while(_receiveRequests.size()!=0)
	{
		...
		while(it!=_receiveRequests.end())
		{
			...
			if(finished)
			{
				...
				log_DEBUG(logName.str(), _world.getWallTime() << " receive request finished, lacking: " << _receiveRequests.size() << " requests");

				const Rectangle<int> & overlapZone = receive->_overlap;
				log_DEBUG(logName.str(), _world.getWallTime() << " processing request for overlap: " << overlapZone << " raster: " << receive->_rasterName << " for max values");
				for(size_t i=0; i<receive->_data.size(); i++)
				{
					...
					if(updateMaxValues)
					{
						log_EDEBUG(logName.str(), "\t" << _world.getWallTime() << " step: " << _step << " receive index: " << index << " max value: " << receive->_data.at(i));
						_world.getDynamicRaster(receive->_rasterName).setMaxValue(index, receive->_data.at(i));
					}
					else
					{
						log_EDEBUG(logName.str(), "\t" << _world.getWallTime() << " step: " << _step << " receive index: " << index << " current value: " << receive->_data.at(i));
				...
				if(updateMaxValues)
				{
					log_DEBUG(logName.str(), _world.getWallTime() << " receive request finished for max values of raster: " <<  receive->_rasterName);
				}
				else
				{
					log_DEBUG(logName.str(), _world.getWallTime() << " receive request finished for current values of raster: " <<  receive->_rasterName);
				}
				delete receive;
			}
		}
...
void SpacePartition::executeAgents()
{
	...
	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " has executed update overlap");
	_executedAgentsHash.clear();

	std::stringstream logNameMpi;
	logNameMpi << "simulation_" << _id;

	log_DEBUG(logName.str(), _world.getWallTime() << " step: " << _step << " executing sections");
	for(int sectionIndex=0; sectionIndex<4; sectionIndex++)
	{
		stepSection(sectionIndex);
		log_DEBUG(logNameMpi.str(), _world.getWallTime() << " executing step: " << _step << " and section: " << sectionIndex << " has been executed");
		receiveAgents(sectionIndex);
		log_DEBUG(logNameMpi.str(), _world.getWallTime() << " executing step: " << _step << " and section: " << sectionIndex << " has received agents");

		sendGhostAgents(sectionIndex);
		log_DEBUG(logNameMpi.str(), _world.getWallTime() << " executing step: " << _step << " and section: " << sectionIndex << " sent ghosts");
		receiveGhostAgents(sectionIndex);
		log_DEBUG(logNameMpi.str(), _world.getWallTime() << " executing step: " << _step << " and section: " << sectionIndex << " received ghosts");

		sendOverlapZones(sectionIndex);
		log_DEBUG(logNameMpi.str(), _world.getWallTime() << " executing step: " << _step << " and section: " << sectionIndex << " sent overlap");
		receiveOverlapData(sectionIndex);
		log_DEBUG(logNameMpi.str(), _world.getWallTime() << " executing step: " << _step << " and section: " << sectionIndex << " received overlap" );
...
{	
	...
	log_INFO(logName.str(), _world.getWallTime() << " simulation finished");
	if(_finalize)
	{
...
const Rectangle<int> & SpacePartition::getOverlapBoundaries() const
{
...
Rectangle<int> SpacePartition::getInternalOverlap( const int & id ) const
{
	...
	std::stringstream logName;
	logName << "MPI_raster_world_" << _id;
	log_DEBUG(logName.str(), _world.getWallTime() << " internal overlap with: " << id << " and diff: " << diff << " - " << result);
	return result;
}
...
Rectangle<int> SpacePartition::getExternalOverlap( const int & id) const
{
	...
	std::stringstream logName;
	logName << "MPI_raster_world_" << _id;
	log_DEBUG(logName.str(), _world.getWallTime() << " external overlap with: " << id << " and diff: " << diff << " - " << result);
	return result;
}
...
Rectangle<int> SpacePartition::getOverlap( const int & id, const int & sectionIndex) const	
{
	...
	std::stringstream logName;
	logName << "MPI_raster_world_" << _id;
	log_DEBUG(logName.str(), _world.getWallTime() << " overlap with: " << id << " and diff: " << diff << " in section Index: " << sectionIndex << " - " << result);
	return result;
}
...
const Point2D<int> & SpacePartition::getLocalRasterSize() const
{
