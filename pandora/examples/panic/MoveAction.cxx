...
{	
	...
	int value = numAgentsPreviousCell*_agentCompressionWeight*(1+(adjacentWallsPreviousCell*_wallCompressionWeight));
	...
	value = numAgentsNewCell*_agentCompressionWeight*(1+(adjacentWallsNewCell*_wallCompressionWeight));	
	...
	for(neighbor._x=_position._x-_panicRadius; neighbor._x<=_position._x+_panicRadius; neighbor._x++)
	{
		...
		{
			if(!agent.getWorld()->checkPosition(neighbor) || agent.getWorld()->getValue(eObstacles, neighbor)==1)
			{
				continue;
			}
