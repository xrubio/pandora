...
Warrior::Warrior( const std::string & id/*, const int & rank, const int & cohesionRating, const int & cohesionDistance, const int & accuracy, const int & reloadingTime*/ ) : Agent(id) //, _isBlueSide(isType("blueWarrior")), _rank(rank), _stress(0), _threshold(1000), _fireDistance(160), _cohesionRating(cohesionRating), _cohesionDistance(cohesionDistance), _moving(true), _routing(false), _reloadingTime(reloadingTime), _timeUntilLoaded(0), _accuracy(accuracy), _delayBeforeFirstFire(0)
{
}
...
void Warrior::move()
{
	if(_rank==1)
	{
		if(_isBlueSide)
		{
			...
			for(front._y=_position._y+_fireDistance-10; front._y>_position._y; front._y--)
			{
				...
				if(agent->exists() && agent->isType("redWarrior"))
				{
					_moving = false;
		...
		else
		{
			...
			{	
				...
				Agent * agent = agents.at(0);
				if(agent->exists() && agent->isType("blueWarrior"))
				{
					_moving = false;
					//std::cout << "unit: " << this << " in rank: " << _rank << " stops" << std::endl;
					return;
	...
	else
	{
		...
		for(int i=1; i<11; i++)
		{
			...
			{
				Warrior * forwardWarrior = (Warrior*)forward;
				if(forwardWarrior->isMoving())
				{
					Engine::Point2D<int> newPos = _position;
					newPos._y += advance;
					if(_world->checkPosition(newPos))
...
void Warrior::updateStress()
{
	std::string enemyType = "redWarrior";
	std::string friendType = "blueWarrior";
	if(!_isBlueSide)
	{
		enemyType = "blueWarrior";
		friendType = "redWarrior";
	}
	...
	if(agent->isType(getType()))
	{
		...
		else
		{
			...
			else
			{
				...
				{
					_stress = _threshold*0.9f;
					Warrior * soldier = (Warrior*)agent2;
					soldier->addStress(20);
					_routing = false;
					return;				
				}
...
void Warrior::casualty()
{
	// we disperse the stress of seeing a casualty unit to other soldiers near it.
	Engine::World::AgentsVector affectedWarrior = _world->getNeighbours(this, _cohesionDistance, getType());
	for(Engine::World::AgentsVector::iterator it=affectedWarrior.begin(); it!=affectedWarrior.end(); it++)
	{
		Agent * agent = (*it);
		if(agent->isType("blueWarrior") || agent->isType("redWarrior"))
		{
			Warrior * soldier = (Warrior*)(*it);
...
/*
float Warrior::getThreatRating( const int & distance )
{
	if(distance>(_fireDistance*1.5f))
	{
		std::stringstream oss;
		oss << "Warrior::getThreatRating error! distance: " << distance;
...
void Warrior::addStress( const int & value )
{
	if(_routing)
	{
		return;
...
int Warrior::getCohesionRating()
{
	return _cohesionRating;
}

bool Warrior::isRouting()
{
	return _routing;
}

bool Warrior::isMoving()
{
	return _moving;
}
*/
void Warrior::serialize()
{
	/*
...
void Warrior::registerAttributes()
{
	/*
...
/*
void Warrior::setDelayBeforeFirstFire( const int & delayBeforeFirstFire )
{
	_delayBeforeFirstFire = delayBeforeFirstFire;
}
	
bool Warrior::firedThisStep()
{	
	if(_reloadingTime==_timeUntilLoaded && _delayBeforeFirstFire==0)
	{
		return true;
